class Dashing.Trauma extends Dashing.Widget
  ...
  formatNumberX = (number) ->
      if number == Math.floor(number)
          #number = ''
      else
          number = 'Room ' + Math.floor(number)
      return number

  formatMap = (number) ->
    map = { 1: '30T', 3: '31T', 5: '32T', 7: '33T'}
    return map[number]
  ...
  @accessor 'current', ->
    ...
    answer = moment.duration({'minutes' : answer})
    answer = if answer < 1 then '' else answer.humanize().toUpperCase()

    switch answer.substring(0,2)
      when 'AN' then answer = "1 " + answer.substring(3)
  ...
  _createGraph: ->
    ...
    # 80% of the actual W/H for better visuals
    width  = width * .8
    height = height * .8
